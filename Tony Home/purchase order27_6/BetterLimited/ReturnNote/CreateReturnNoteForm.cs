...
namespace BetterLimited
{
    public partial class CreateReturnNoteForm : Form
    {
            ...
            if (e.ColumnIndex == 2)
            {
                try
                {
                    foreach (DataGridViewRow row in dataGridView_OrderItemTable.Rows) {
                        if (row.Cells[3].Value.ToString().Equals(qty.ToString() ) )
                        {
                            MessageBox.Show("his order have not that much item quantity", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                    }
        ...
        {
            if (dataGridView_ReturnNote.Rows.Count == 0) {
                MessageBox.Show("You have items in Return Note.");
                return;
            }
