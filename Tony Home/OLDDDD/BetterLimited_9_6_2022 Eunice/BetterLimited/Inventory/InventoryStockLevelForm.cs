...
namespace BetterLimited
{
    public partial class InventoryStockLevelForm : Form
    {
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    sql = "SELECT ItemID, ItemName, Quantity , LowLevelQty FROM inventorystocklevel WHERE ItemID LIKE '%" + this.txtSearch.Text + "%'";
                    showData(sql);
                    break;
                case 1:
                    sql = "SELECT ItemID, ItemName, Quantity , LowLevelQty FROM inventorystocklevel WHERE ItemName LIKE '%" + this.txtSearch.Text + "%'";
                    showData(sql);
                    break;
            }
        }
