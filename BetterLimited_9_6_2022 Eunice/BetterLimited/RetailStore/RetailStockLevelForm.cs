...
namespace BetterLimited
{
    public partial class RetailStockLevelForm : Form
    {
        ...
        DataTable dt;
        string sql = "SELECT itemID, itemName, Quantity, LowLevelQty FROM retailstocklevel WHERE storeID = 1";
        ...
        public void showData(string sql)
        {
            try
            {
                ...
                dgvRetailStockLevel.DefaultCellStyle.Font = new Font("Segoe Print", 9);
                checkStock();
                closeConnection();
            }
            catch (Exception ex) 
        ...
        private void checkStock()
        {
            ...
            for (int i = 0; i < dgvRetailStockLevel.Rows.Count; i++)              
            {
                ...
                if (quantity < lowLevel)
                {
                    ...
                    dgvRetailStockLevel.Rows[i].Cells[4].Style.Font = new Font("Segoe Print", 12, FontStyle.Bold);
                }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    sql = "SELECT ItemID, ItemName, Quantity , LowLevelQty FROM retailstocklevel WHERE storeID = '" + storeID + "' AND ItemID LIKE '%" + this.txtSearch.Text + "%'";
                    showData(sql);
                    break;
                case 1:
                    sql = "SELECT ItemID, ItemName, Quantity , LowLevelQty FROM retailstocklevel WHERE storeID = '" + storeID + "' AND ItemName LIKE '%" + this.txtSearch.Text + "%'";
                    showData(sql);
                    break;
            }
        }
        ...
        {
            storeID = "1";
            sql = "SELECT itemID, itemName, Quantity, LowLevelQty FROM retailstocklevel WHERE storeID = 1";
            showData(sql);
        ...
        {
            storeID = "2";
            sql = "SELECT itemID, itemName, Quantity, LowLevelQty FROM retailstocklevel WHERE storeID = 2";
            showData(sql);
